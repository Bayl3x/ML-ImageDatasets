...
#n_comp = number of sub matrices to be produced
def get_NMF(img_vector, n_comp):
    #Create the model with the appropriate parameters
    model = NMF(n_components=n_comp, init='random', random_state=0)
    #Fit the model
    W = model.fit_transform(img_vector)
    #Get the resulting matrices
    H = model.components_
...
def output_to_file(image_str, feature, output_filename, info_file, minKeypoints=0):    
...
def process_images(folder, output_filename, feature_model, info_file):
    minKeypoints = 0
    if feature_model == 'SIFT' or feature_model == 'SIFT':
        img = load_image(folder + os.listdir(folder)[0])
        minKeypoints = int(len(get_SIFT(img)) / 128)
        for filename in os.listdir(folder):
            img = load_image(folder + filename)
            numKeypoints = int(len(get_SIFT(img)) / 128)
            if numKeypoints < minKeypoints:
                minKeypoints = numKeypoints
    ...
    for filename in os.listdir(folder):
        if filename.endswith('.jpg'):
            ...
            output_to_file(os.path.join(folder, filename), feature_model, output_filename, info_file, minKeypoints)
        else:
            continue
...
def normalize_cm(dataMatrix):
    normalizedMatrix = dataMatrix
    for i in range(len(dataMatrix)):
        mean = dataMatrix[i][0::3]
        std = dataMatrix[i][1::3]
        skew = dataMatrix[i][2::3]
        for j in range(len(dataMatrix[i])):
            if j % 3 == 0 or j == 0:
                normalizedMatrix[i][j] = (dataMatrix[i][j] - min(mean)) / (max(mean) - min(mean))
            if j % 3 == 1 or j == 1:
                normalizedMatrix[i][j] = (dataMatrix[i][j] - min(std)) / (max(std) - min(std))
            if j % 3 == 2 or j == 2:
                normalizedMatrix[i][j] = (dataMatrix[i][j] - min(skew)) / (max(skew) - min(skew))
...
def display_dls(dm, dls, images, image_folder):
    ...
    for ls in ordereddls:
        ...
        for i in range(len(ls)):
            ...
            imsp.imshow(load_image_rgb(image_folder+images[ls[i][0]]))
...
def display_fls(dm, fls, images, image_folder):
    for ls in fls:
        ...
        plt.imshow(load_image_rgb(image_folder + images[bestImageIndex]))
...
def negative_cosine_similarity_mv(dataMatrix, feature_vector):
    ...
    return -1 * np.asarray((np.divide(np.dot(dataMatrix, feature_vector.T),np.linalg.norm(dataMatrix, axis=1)*np.linalg.norm(feature_vector)))*-1, dtype=np.float32)
...
def display_subject_images(subjectID, dist, csv_file, image_folder, feature_model):
    ...
    for i in range(len(image_names)):
        ...
        imsp.imshow(load_image_rgb(image_folder+image_names[i]))
        imsp.axes.get_xaxis().set_visible(False)
        imsp.axes.get_yaxis().set_visible(False)
    plt.show()
###############################
